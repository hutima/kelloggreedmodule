...
- ONE WORD NODE PER WORD — NO WRAPPERS: make exactly one "word" node per word. A node holds MORE than one token ONLY for a single inseparable verb form (see VERB PHRASE) — NEVER for a verb plus its object, a verb plus a following complement verb, or a word plus a modifier; those are ALWAYS separate nodes joined by a relation. Do NOT create a "phrase" node that merely repeats its children's tokens, and do NOT wrap a compound in an empty role node — put the role on the HEAD word and attach the rest to it (see COORDINATION). You rarely need a "phrase" node at all.
- ISOLATE EVERY WORD'S FUNCTION — do NOT leave a multi-word predicate (or any phrase) as one undivided block: break it into its verb, its object/complement, and its modifiers, each its own node. A verb and its DIRECT OBJECT are separate nodes joined by "directObject". Negation and other adverbs ("not", 不, 别, 不是 in its negating sense) are their own "adverbial" node under the word they modify.
...
- VERSE NUMBERS & OTHER NON-TEXT TOKENS ARE NOT DIAGRAMMED: if you spot a token that is NOT part of the sentence itself — a verse or chapter number ("16"), a verse reference ("1:16", "3:1-2", "v.3", "5b"), a footnote / cross-reference marker, a heading, or any editorial insertion — EXCLUDE it exactly like punctuation: it gets NO node and appears in NO relation. Leave it in the "tokens" list (pos "unknown") but out of "nodes" and "relations" entirely, so the diagram shows only the actual words of the text. (A numeral that is a genuine WORD of the sentence — "five loaves", "the twelve", "year 2000" — is NOT a verse number: diagram it normally as a "numeral".)
- COORDINATION OF WORDS ("X and Y" — compound subjects, objects, or verbs WITHIN one clause): do NOT wrap the parts in a parent node. Make the FIRST part the head that carries the role (e.g. "subject"); attach each other part to that head with a "conjunct" relation and each conjunction word with a "coordinator" relation.
- COORDINATION OF CLAUSES (two or more FULL clauses/sentences joined by "and", "but", "or", "so", ";" …, e.g. "the court acted, AND they tried him"): make a HEADLESS parent { "kind":"clause", "clauseType":"coordinate", "tokens":[] }, attach EACH full clause to it with a "conjunct" relation, attach each joining conjunction with a "coordinator" relation, and set "rootId" to that coordinate clause. This coordinate clause is the ONE wrapper you SHOULD create — it is a real clause, not an empty phrase, and it draws as the compound-sentence spine with the conjunction on the joining bar. Do NOT instead make the first clause the root and hang the others under it: that renders as SUBORDINATION and strands the conjunction as a stray slant.
- A VERB PHRASE is ONLY a tense/aspect/voice AUXILIARY plus its main verb that together are ONE finite verb ("have lighted", "will go", "is being built"): make ONE "predicate" node holding both verb tokens. This does NOT cover a MODAL or matrix verb (want, must, can, try, begin, 要 "want to", 能 "can") followed by another verb — there the second verb is a COMPLEMENT with its OWN node, attached to the first with "directObject" (or, if it heads a clause, as a complement clause). Never merge two verbs that mean "verb + what-is-wanted/done". Do not invent roles like "auxiliary" or "head".
...
- "rootId" is the TOP clause: the single main clause, or — when the sentence is compound (two or more coordinated main clauses) — the headless "coordinate" clause that joins them (see COORDINATION OF CLAUSES). A SUBORDINATE clause (adverbial "although/when/because…", relative "who/which…", complement "that…") is NOT a coordinate member: attach it to the clause or word it modifies with "adverbial"/"adjectival"/"directObject", and keep its introducing subordinator as a "conjunction"-role word INSIDE that subordinate clause (it draws as the clause's introductory connective).
...
EXAMPLE — a modal/matrix verb + a NEGATED complement "not want to revolt" (each word isolated — do NOT lump the verbs into one node):
  nodes:  { "id":"neg","kind":"word","role":"adverbial","tokens":["not"] }, { "id":"vwant","kind":"word","role":"predicate","tokens":["want"] }, { "id":"vcomp","kind":"word","role":"directObject","tokens":["revolt"] }
  relations:  { "type":"predicate","head":"c0","dependent":"vwant" }, { "type":"adverbial","head":"vwant","dependent":"neg" }, { "type":"directObject","head":"vwant","dependent":"vcomp" }

EXAMPLE — a COMPOUND SENTENCE "The court acted, and they tried him" (a HEADLESS "coordinate" clause joins the two FULL clauses; "and" is its coordinator — NOT the first clause as root):
  nodes:  { "id":"cc","kind":"clause","clauseType":"coordinate","tokens":[] }, { "id":"a","kind":"clause","clauseType":"independent","tokens":[] }, { "id":"b","kind":"clause","clauseType":"independent","tokens":[] }, { "id":"cj","kind":"word","role":"coordinator","tokens":["and"] }  … plus each clause's own subject/predicate word nodes …
  relations:  { "type":"conjunct","head":"cc","dependent":"a" }, { "type":"conjunct","head":"cc","dependent":"b" }, { "type":"coordinator","head":"cc","dependent":"cj" }, { "type":"subject","head":"a","dependent":"…" }, { "type":"predicate","head":"a","dependent":"…" }, …  ; rootId: "cc"
...
BEFORE YOU REPLY, check each of these and FIX any that fail (this is where parses usually go wrong — dropped or duplicated words):
  1. COUNT — every real WORD of the sentence (every token that is NOT punctuation and NOT a verse number / non-text marker) appears in EXACTLY ONE node's "tokens": none dropped, none in two nodes.
  2. Every "clause" node has "tokens": [] (its words live in their own word nodes, not on the clause).
  3. No punctuation token — and no verse number or other non-text token — has a node or a relation.
  4. Every relation "head"/"dependent" is a node id that exists, and "rootId" is your top clause.
  5. Two coordinated FULL clauses hang off a HEADLESS "coordinate" clause (which is "rootId") — not off the first clause — with "and"/"but"/… as its "coordinator".
